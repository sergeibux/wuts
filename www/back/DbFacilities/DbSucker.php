...
//           loggerLog('speciesCount = ' . $totalAmount . ' ... 3', 'mnhn_suscker()', 'dboLog.txt', true, false);
            ...
            $dest = $_SERVER["HTTP_HOST"] . '/back/tests/links.php?totalAmount=' . $totalAmount . '&limit=' . LIMIT;
            header('Location: http://' . $dest);  
            exit();
        }
//         $last = $first + LIMIT;
//         for ($i = $first; $i < $last; ){
            $jsonTab = $this->getSpeciesByIdRange($dataSrc, $first, LIMIT);
            echo "<hr><h4>species from $first to " . (($first + LIMIT) - 1) . "</h4>";
            $tabSize = sizeof($jsonTab);
            for ($j=0; $j<$tabSize; $j++){
...
//                 echo "<br>Create new branch ??<br>";
                ...
                loggerLog('DEBUG:DbSucker / id = ' . $family->getId(), 'dbSucker()', 'dboLog.txt', true, false);
